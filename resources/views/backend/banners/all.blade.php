...
@push('scripts')
    ...
    <script type="text/javascript">
        ...
        $(document).on('click', '.banner_status', function() {
            ...
            $.ajax({
                type: 'POST',
                url: route('banners.update.status'),
                data: {
                    id: id,
                    is_active: is_active
                },
                success: function(data) {
                    ...
                    if (is_active == 1) {
                        $('.banner_status#switch' + id).attr('value', 0);
                    } else {
                        $('.banner_status#switch' + id).attr('value', 1);
                    }
        ...
        $(document).on("click", ".remove", function(event) {
            var flag = confirm('Are You Sure want to Remove Banner?');
            if (flag) {
                var id = $(this).data('id');
                $.ajax({
