    ...
    <script type="text/javascript">
        $(function() {
            var table = $('.data-table').DataTable({
                processing: true,
                serverSide: true,
                ajax: route('sectioncontents.index'),
                columns: [{
                    data: 'id',
                    name: 'id'
                },{
        ...
        $(document).on('click', '.sectioncontent_status', function() {
            var id = $(this).attr('data-id');
            var is_active = $(this).val();
            $.ajax({
                type: 'POST',
                url: route('sectioncontent.update.status'),
                data: {
                    id: id,
                    is_active: is_active
                },
                success: function(data) {
                    ...
                    if (is_active == 1) {
                        $('.sectioncontent_status#switch' + id).attr('value', 0);
                    } else {
                        $('.sectioncontent_status#switch' + id).attr('value', 1);
                    }
        ...
        $(document).on("click", ".remove", function(event) {
            var flag = confirm('Are You Sure want to Remove Anncoument?');
            if (flag) {
                var id = $(this).data('id');
                $.ajax({
                    url: route('sectioncontents.destroy', id),
                    type: 'DELETE',
                    data: {
                        id: id
                    },
