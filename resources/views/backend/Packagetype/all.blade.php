    ...
    <script type="text/javascript">
        $(function() {
            var table = $('.data-table').DataTable({
                processing: true,
                serverSide: false,
                ajax: route('package-types.index'),
                columns: [{
                    data: 'id',
                    name: 'id'
                }, {
        ...
        $(document).on('click', '.categories_status', function() {
            ...
            $.ajax({
                type: 'POST',
                url: route('package-type.update.status'),
                data: {
                    id: id,
                    is_active: is_active
                },
        ...
        $(document).on("click", ".remove", function(event) {
            var flag = confirm('Are You Sure want to Remove Package Type?');
            if (flag) {
                var id = $(this).data('id');
                $.ajax({
                    url: route('package-types.destroy', id),
                    type: 'DELETE',
                    data: {
                        id: id
                    },
